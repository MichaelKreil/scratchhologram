...
exports.generator = {
	getRandomPoints: function (count, size) {
		if (!count) count = 30;
		if (!size)  size  = 100;
	...
	getMengerSponge: function (depth, s) {
		if (!s) s  = 100;
		var points = [];
	...
	getFromCT: function (folder) {
		var i = 0;
		var files = [];
		do {
			i++;
			var filename = folder+'IM'+(1000000+i).toFixed(0).substr(1);
			var exists = fs.existsSync(filename);
			if (exists) files.push(filename);
		} while (exists);

		var points = [];

		function addPoint(x,y,z) {
			points.push({ x:x, y:y, z:z+Math.random()/10 });
		}

		files.forEach(function (filename, index) {
			var data = fs.readFileSync(filename);
			var index0 = data.length - 512*512*2;
			for (var y = 0; y < 512; y++) {
				for (var x = 0; x < 512; x++) {
					var i = index0 + (y*512 + x)*2;
					var v = data.readUInt16LE(i);
					v = v - 1300;
					if (v > Math.random()*20000) addPoint(x,-y,index*10);
				}
			}
		})
...
exports.Points = function (points) {
	var me = {};
	var data = {
	...
	me.calcCurves = function (blur) {
		var measurements = 25;
		var dots = data.dots;

		dots.forEach(function (dot) { dot.curve = calcCurve(dot.point) });
		...
		if (blur) {
			...
			var depthBuffer;
			function get(x, y) {
				if (!depthBuffer[x] || !depthBuffer[x][y]) return [];
				return depthBuffer[x][y];
			}
			function set(x, y, z) {
				if (!depthBuffer[x]) depthBuffer[x] = [];
				if (!depthBuffer[x][y]) depthBuffer[x][y] = [];
				depthBuffer[x][y].push({x:x,y:y,z:z});
			}

			for (var i = 0; i <= measurements*2; i++) {
				depthBuffer = [];
				//console.log(i);
				for (var k0 = 0; k0 < dots.length; k0++) {
					var dot = dots[k0];
					var show = true;
					var xi = Math.round(dot.curve[i].x);
					var yi = Math.round(dot.curve[i].y);

					get(xi,yi).forEach(function (point) {
						var d = vec.distanceXY(dot.curve[i], point);
						if ((d < blur) && (point.z >= dot.point.z)) {
							show = false;
						}
					})

					dot.curve[i].show = show;

					if (show) {
						for (var dx = -blur-1; dx < blur+1; dx++) {
							for (var dy = -blur-1; dy < blur+1; dy++) {
								set(xi+dx, yi+dy, dot.point.z);
							}
						}
					}
				}
			...
			for (var k = 0; k < dots.length; k++) {
				var i0 = 0, i1 = -1;
				var maxLength = 0, maxI0 = 0, maxI1 = -1;
				for (var i = 0; i <= measurements*2; i++) {
					if (dots[k].curve[i].show) {
						i1 = i;
					} else {
						if (i1-i0+1 > maxLength) {
							maxLength = i1-i0+1;
							maxI0 = i0;
							maxI1 = i1;
						}
				...
				if (maxLength > 3) {
					dots[k].curve = dots[k].curve.slice(maxI0, maxI1);
					n++;
				} else {
					dots[k].curve = [];
				}
			...
			console.log('Number of Points left: '+n);
		}
...
var vec = {
	...
	distanceXY: function (p1, p2) { return sqrt(sqr(p1.x-p2.x) + sqr(p1.y-p2.y)) },
	distanceXYSqr: function (p1, p2) { return sqr(p1.x-p2.x) + sqr(p1.y-p2.y) },
